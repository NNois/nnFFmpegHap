...
Choose one of two build options:

#### Option A: Standard Build (Faster, MSYS2 required)
```bash
./rebuild-with-hap.sh
```
- Smaller executables (~30MB)
- **Requires MSYS2 to be in PATH** when running from PowerShell/CMD
- Works fine in MSYS2 terminal

#### Option B: Portable Build (Recommended for apps)
```bash
./rebuild-static.sh
```
...
- Wrong directory. Run: `cd /j/2500_AdToolBox/ffmpeg/FFmpeg-8.0`

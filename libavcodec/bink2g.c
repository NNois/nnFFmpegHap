                            ...
                            int q, const uint16_t qmat[4][64])
{
    ...
    for (int i = 0; i < 4; i++, cbp >>= 1) {
        ...
        while (idx < 64) {
            next--;
            if (next < 1) {
                skip = get_vlc2(gb, skip_vlc, BINK2_VLC_BITS, 1);
                if (skip < 0)
                                    ...
                                    int flags)
{
    ...
    *prev_cbp = cbp = bink2g_decode_cbp_luma(c, gb, *prev_cbp);
    ...
    bink2g_decode_dc(c, gb, dc, 1, q, c->is_ver25 ? -2047 : -1023, c->is_ver25 ? 2047 : 1023, 0xA8);
                                      ...
                                      int flags)
{
    ...
    *prev_cbp = cbp = bink2g_decode_cbp_chroma(gb, *prev_cbp);
    ...
    bink2g_decode_dc(c, gb, dc, 0, q, c->is_ver25 ? -2047 : -1023, c->is_ver25 ? 2047 : 1023, 0xA8);
                            ...
                            MVectors *mv)
{
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < num_mvs; j++) {
            int val = get_vlc2(gb, bink2g_mv_vlc, BINK2_VLC_BITS, 1);
                               ...
                               int is_kf, int start, int end)
{
    ...
    int ret = 0, dq, flags;
    int do_alpha = c->has_alpha && c->constant_alpha < 0;

...
set "LIBVPX_INSTALL=C:\ff\ff\build-msvc\libvpx-msvc"
...
"%MSYS_BASH%" -lc "MSVC_BIN_WIN=\"%MSVC_BIN_WIN%\"; WINSDK_BIN_WIN=\"%WINSDK_BIN_WIN%\"; MSBUILD_BIN_WIN=\"%MSBUILD_BIN_WIN%\"; MSVC_BIN=\"$(cygpath -u \"$MSVC_BIN_WIN\")\"; WINSDK_BIN=\"$(cygpath -u \"$WINSDK_BIN_WIN\")\"; MSBUILD_BIN=\"$(cygpath -u \"$MSBUILD_BIN_WIN\")\"; export PATH=\"$MSVC_BIN:$WINSDK_BIN:$MSBUILD_BIN:$PATH\"; export INCLUDE=\"$INCLUDE\"; export LIB=\"$LIB\"; rm -rf build-msvc; mkdir build-msvc; cd build-msvc; ../configure --target=x86_64-win64-vs17 --disable-examples --disable-tools --disable-unit-tests --disable-docs --disable-webm-io --enable-vp8 --enable-vp9 --prefix=/c/ff/ff/build-msvc/libvpx-msvc && make -j1 vpx.vcxproj && sed -i 's/MultiThreaded<\\/RuntimeLibrary>/MultiThreadedDLL<\\/RuntimeLibrary>/g' vpx.vcxproj && sed -i 's/MultiThreadedDebug<\\/RuntimeLibrary>/MultiThreadedDebugDLL<\\/RuntimeLibrary>/g' vpx.vcxproj && make -j$(nproc) && make install" || exit /b 1

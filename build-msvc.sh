...
LOG_FILE="${FFMPEG_BUILD_LOG:-build-msvc.log}"
: > "$LOG_FILE"
...
echo "=========================================="
echo "FFmpeg - Unity Static Build (MSVC)"
echo "=========================================="
echo ""
...
if [ ! -f "$ZLIB_INSTALL/lib/zlib.lib" ] && [ ! -f "$ZLIB_INSTALL/lib/zlibstatic.lib" ]; then
  echo "ERROR: zlib not found. Run build-msvc-zlib.bat first."
  exit 1
fi
echo "  zlib: OK"
...
if [ ! -f "$VPX_INSTALL/lib/x64/vpx.lib" ] && [ ! -f "$VPX_INSTALL/lib/vpx.lib" ] && [ ! -f "$VPX_INSTALL/lib/libvpx.lib" ]; then
  echo "ERROR: libvpx not found. Run build-msvc-libvpx.bat first."
  exit 1
fi
echo "  libvpx: OK"

SDL2_INSTALL="/c/ff/ff/build-msvc/sdl2-msvc"
if [ ! -f "$SDL2_INSTALL/lib/SDL2-static.lib" ] && [ ! -f "$SDL2_INSTALL/lib/SDL2.lib" ]; then
  echo "ERROR: SDL2 not found. Run build-msvc-sdl2.bat first."
  exit 1
fi
echo "  SDL2: OK"
...
if [ ! -f "$DAV1D_INSTALL/lib/dav1d.lib" ]; then
  echo "  dav1d: NOT FOUND (optional, run build-msvc-dav1d.bat to enable)"
  DAV1D_ENABLED=0
else
  echo "  dav1d: OK"
  DAV1D_ENABLED=1
...
#!/bin/sh
prefix="C:/ff/ff/build-msvc/sdl2-msvc"
libdir="$prefix/lib"
...
target_prefix="./build-msvc"

# Convert MSYS2 paths to Windows paths for MSVC
ZLIB_INSTALL_WIN="C:/ff/ff/build-msvc/zlib-msvc"
ZLIB_CFLAGS="-IC:/ff/ff/build-msvc/zlib-msvc/include"
ZLIB_LDFLAGS="-LIBPATH:C:/ff/ff/build-msvc/zlib-msvc/lib"

# libvpx (required for VP8/VP9 with alpha)
VPX_ROOT_WIN="${VPX_ROOT:-C:\\ff\\ff\\build-msvc\\libvpx-msvc}"
...
VPX_CFLAGS="-IC:/ff/ff/build-msvc/libvpx-msvc/include"
if [ -f "$VPX_ROOT_UNIX/lib/x64/vpxmd.lib" ]; then
  VPX_LDFLAGS="-LIBPATH:C:/ff/ff/build-msvc/libvpx-msvc/lib/x64"
else
  VPX_LDFLAGS="-LIBPATH:C:/ff/ff/build-msvc/libvpx-msvc/lib"
fi
# Add vpx lib for FFmpeg's check_lib tests
VPX_EXTRA_LIBS="vpx.lib"

# SDL2 (for ffplay)
SDL2_CFLAGS="-IC:/ff/ff/build-msvc/sdl2-msvc/include -IC:/ff/ff/build-msvc/sdl2-msvc/include/SDL2"
SDL2_LDFLAGS="-LIBPATH:C:/ff/ff/build-msvc/sdl2-msvc/lib"
if [ -f "/c/ff/ff/build-msvc/sdl2-msvc/lib/SDL2-static.lib" ]; then
...
./configure \
    ...
    --enable-swscale \
    --enable-swresample \
    --enable-demuxer=mov,mp4,matroska,webm,webp,image2,image2pipe,ogg,opus,mp3,wav,aac,aiff \
    --disable-decoder=vp8,vp9 \
    --enable-decoder=h264,hevc,libvpx_vp8,libvpx_vp9,libdav1d,hap,prores,png,webp,mjpeg,targa,pcm_s16le,pcm_s24le,pcm_s32le,pcm_f32le,pcm_f64le,pcm_u8,aac,alac,vorbis,opus,mp3,mp3float \
...
echo "  ./build-msvc-copy-unity.sh"
echo ""
if [ -t 0 ]; then
  echo "Run it now? [Y/n]"
  read -r RUN_COPY
  if [ -z "$RUN_COPY" ] || [ "$RUN_COPY" = "y" ] || [ "$RUN_COPY" = "Y" ]; then
      ./build-msvc-copy-unity.sh
  fi
else
  echo "Skipping Unity copy (non-interactive shell)."
fi
